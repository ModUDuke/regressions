...
# Safety domain: assessment of "a broad set of healthcare activities that affect patients’ well-being."
# https://www.medicare.gov/HospitalCompare/Data/safety-domain.html
# Mortality: Death rate for pneumonia patients in 30 days after entering the hospital
# https://www.medicare.gov/hospitalcompare/Data/Death-rates.html

# I HAVE DOWNLOADED THESE DATASETS, HAVEN'T ADDED THEM TO DATACAMP SERVERS THOUGH. ONE NEEDS TRIMMING DOWN TO 10MB

root <- # SET ROOT DIR HERE
path <- paste0(root,"Complications and Deaths - Hospital.csv")
df.tmp1 <- read.csv(path)
df.mortality <- df.tmp1[df.tmp1$Measure.ID=="MORT_30_HF",]
df.ranking <- read.csv("hvbp_tps_11_07_2017.csv")
df.tmp2 <- merge(df.mortality, df.ranking, by.x = "Provider.ID", by.y = "Provider.Number")
hospitals <- data.frame(id = df.tmp2$Provider.ID, name = df.tmp2$Hospital.Name.x,
                        city = df.tmp2$City.x, State = df.tmp2$State.x,
                        mortality = as.numeric(levels(df.tmp2$Score))[df.tmp2$Score],
                        performance = as.numeric(levels(df.tmp2$Weighted.Safety.Domain.Score))[df.tmp2$Weighted.Safety.Domain.Score])
hospitals <- hospitals[!is.na(hospitals$mortality) & 
                       !is.na(hospitals$performance),]
```

*** =sample_code
```{r}
# 1) You will explore the relationship between hospital performance and mortality. Calculate the correlation coefficient between these variables.

#---- Question 1-------------------------------------#
      Solution1<-cor()
#----------------------------------------------------#

# 2) Let's further analyze the relationship between mortality and performance. Plot the two variables on the same graph, and fit a line as well.

#---- Question 2-------------------------------------#
      Solution2<-ggplot(data= , aes(x= , y= )) +
                        geom_point() +
                        geom_smooth(method= )
#----------------------------------------------------#
...
***=solution
Solution1<-cor(hospitals$mortality, hospitals$performance)
Solution2<-ggplot(data=hospitals, aes(x=performance, y=mortality)) +
                  geom_point() +
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:5f52195dc6
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:6650875c79
