...
Hospitals<-read.csv("https://assets.datacamp.com/production/repositories/1142/datasets/0dd17890b0be636af72dd8d54c3e2dc78a4191be/Question Hospital Data.csv")
```
...
# Curious! Hospitals with a higher "Total Performance Score" are better hospitals, so you would think that the correlation between hospital quality and pneumonia mortality rate in this data would be a negative correlation.

# 2) Let's further analyze the relationship between the mortality score on pneumonia and a hospital's total performance score by looking at a graph with a trend line that reflects this correlation. Plot these two variables on the same graph, and fit a line as well, by inserting the dataframe name and variable names into the following line of code.

	ggplot(data= , aes(x= , y= )) + geom_point() + geom_smooth(method=lm)

# 3) Good. Now it's time to interpret these findings.  What do our findings imply? 
