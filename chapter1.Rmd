...
## Course Trailer for Causal Inference with R - Regression
...
skills: 1
key: 4070242f87 
video_link: //player.vimeo.com/video/303322440 
```
...
xp: 50
skills: 1
video_link: "//player.vimeo.com/video/293196239"
```
...
Let's say you're a consultant who is on a team analyzing the performance of hospitals. You are given data on hospitals' mortality rates and their safety performance. Specifically, these data include hospitals around the United States in 2017; mortality corresponds to patients' death rates of heart failure within 30 days after entering the hospital, and the hospital's overall safety performance rating (on a scale from 0 to 100).
...
- 4) Decide on the best causal interpretation to explain your findings.
...
Hospitals<-read.csv("https://assets.datacamp.com/production/repositories/1142/datasets/d376bb205b670147f5f072c5a162c1e8288d0b50/Hospitals2.csv")
```
...
# Note: Curious! Hospitals with a higher "Total Performance Score" are better hospitals, so you would think that the correlation between hospital quality and pneumonia mortality rate in this data would be a negative correlation.


# 3) Let's further analyze the relationship between the mortality score on pneumonia and a hospital's total performance score by looking at a graph with a trend line that reflects this correlation. Plot these two variables on the same graph, and fit a line as well, by inserting the dataframe name and variable names into the following line of code.

    ggplot(data= , aes(x= , y= )) + geom_point() + geom_smooth(method=lm)


# 4) Good. Now it's time to interpret these findings.  What do our findings imply? Enter "A", "B", "C", or "D" as Solution3.

# (A) As hospital performance gets better, pneumonia patient mortality stays the same. 
# (B) As hospital performance gets better, pneumonia patient mortality decreases; therefore the performance measure accurately assesses hospitals.
# (C) As hospital performance gets better, pneumonia patient mortality increases; therefore the performance measure is useless.
# (D) As hospital performance gets better, pneumonia patient mortality increases; we need further analyses to find the true causal relationship between these measures.
...
`@sct`
```{r}
msg1 = "It does sound like giving tax breaks means people have more income, and having more income means you can expect more Walmarts in town, but do we really know that is a causal effect and not just a correlation caused by something else? Try again."
msg2 = "No, you can't learn the direct causal effect from this regression as described, because there are so many other social and economic factors to consider in this relationship that Unconfoundedness Assumption is unlikely to be true. In this case, we can just learn the amount of correlation between the variables."
...
`@sct`
```{r}
msg1 = "That's too low. Try again."
msg2 = "That's not it, so try running the numbers again."
...
xp: 50
skills: 1
video_link: "//player.vimeo.com/video/293196243"
```
