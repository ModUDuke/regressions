...
*** =sample_code
```{r}
# 1) As usual, let's first look at the variables in the dataset, which was collected by the researchers on a sample that included NERD gamers along with other people. 
...
# Whether the respondent plays NERD is measured via the Treatment variable. 
...
# Use the summary command on the dataframe `NERD`, and take a few minutes to understand what's in the data.
...
*** =instructions
- 1) Use matching techniques to balance the dataset
- 2) Estimate standard OLS regression model for communication skills (`Communication`) based on all other variables in the matched dataset (`match.NERD`).
- 3) Check the statistical significance of the regression on the matched data.
...
# The following syntax builds a model for estimating "Treatment" by all other control variables in our dataset `NERD`: matchit(Treatment ~ control1+control2+control3+control4+control5, data = NERD). Use that syntax to populate the dataframe `match.it`:
    match.it <-

# Now let's subset the NERD dataset to observations that have a high predicted probability for being in the treatment group. We have generated that code for you,so select the following code and hit the "Run Code" button:
    matched.NERD <- match.data(match.it)[1:ncol(NERD)]
    
# To compare the matched dataset to the original dataset, let's use summary commands on the object that was produced by the "matchit" formula. 
#-------------------- Question 1 --------------------#
      Solution1<-glm()
#----------------------------------------------------#
...
# Let's now estimate a generalized linear regression model for communication skills using our matched dataset (matched.NERD). 
    
#-------------------- Question 2 --------------------#
      Solution2<-glm()
#----------------------------------------------------#

# Is the effect of "Treatment" in Solution2 positive and statistically significant? Use the summary command on Solution2 to check. Answer with "Yes" or "No"
...
```{r}
  ...
  matched.NERD <- match.data(match.it)[1:ncol(NERD)]
  Solution1<-summary(match.it)
  Solution2<-glm(Communication~ ParentEducation+BooksOwned+ReadsBlogs+Siblings+Bilingual+LikesBooks+Treatment,data=matched.NERD)
  Solution3<-"No"
```
*** =sct
```{r}
